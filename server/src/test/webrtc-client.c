...
static GOptionEntry options[] = {
    { "websocket-uri", 'u', 0, G_OPTION_ARG_STRING, &websocket_uri,
      "Websocket URI of webrtc signaling connection" },
    { NULL }
};

#define WEBSOCKET_URI_DEFAULT "ws://localhost:8080/ws"
...
on_answer_created (GstPromise *promise, gpointer user_data)
{
  ...
  gst_structure_get (gst_promise_get_reply (promise),
      "answer", GST_TYPE_WEBRTC_SESSION_DESCRIPTION, &answer, NULL);
...
process_sdp_offer (const gchar *sdp)
{
  ...
  desc = gst_webrtc_session_description_new (GST_WEBRTC_SDP_TYPE_OFFER,
      sdp_msg);
  if (desc) {
    GstPromise *promise;
    ...
    g_signal_emit_by_name (webrtcbin, "create-answer", NULL,
        gst_promise_new_with_change_func (
            (GstPromiseChangeFunc) on_answer_created, NULL, NULL));
  } else {
...
static void
websocket_connected_cb (GObject * session, GAsyncResult * res,
    gpointer user_data)
{
  GError *error = NULL;
  ...
  if (error) {
    ...
    pipeline = gst_parse_launch (
        "webrtcbin name=webrtc ! rtph264depay ! h264parse ! avdec_h264 ! autovideosink",
        &error);
...
int main (int argc, char *argv[])
{
  ...
  soup_session_websocket_connect_async (soup_session,
      soup_message_new (SOUP_METHOD_GET, websocket_uri), NULL, NULL, NULL,
      websocket_connected_cb, NULL);
