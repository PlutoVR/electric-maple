...
websocket_connected_cb(GObject *session, GAsyncResult *res, gpointer user_data)
{
	...
	// FOR RYLIE : This is where our gstreamer plugins (especially the very important
	...
	if (error) {
		...
		U_LOG_E("FRED : make current display=%i, surface=%i, context=%i", (int)vid->state->display,
		...
		// FOR RYLIE : We are STARTING the pipeline. From this point forth, if built with
                      ...
                      enum xrt_stereo_format stereo_format) //
{
	...
	// FOR RYLIE: We'll need this thread/mainloop for the websocket cb to fire below.
	...
	// FOR RYLIE : We're using libsoup here to create our websocket session. We're starting the thread herebelow
...
// FOR RYLIE : This is our entry point in the gst "Driver", a fancy way of saying
...
{
	// FOR RYLIE: The below JNI_OnLoad call MUST NOT be called !! It's just
